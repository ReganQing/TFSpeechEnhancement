...
def training(path_save_spectrogram, weights_path, name_model, training_from_scratch, epochs, batch_size):
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(weights_path + '/model_best.h5', verbose=1, monitor='val_loss', save_best_only=True,
                                 mode='auto')
    ...
    ax.plot(epochs, loss, color='c', label='Training loss')
    ax.plot(epochs, val_loss, color='DarkSeaGreen', label='Validation loss')
    ax.set_title('Training and validation loss' + r'($\alpha=0.8$)')  # + r'($\alpha=$)'
    plt.yscale('log')
    plt.legend(loc=1)
    plt.show()
